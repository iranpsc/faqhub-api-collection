...
class CheckUnCorrectAnswerPermission
{
    /**
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        // Check if the user has already marked an answer for this question as correct
        ...
        if ($answer->user_id == $user->id) {
            return $this->denyAccess('You cannot unmark your own answer as correct.');
        }
        ...
        if (in_array($userLevelSlug, $restrictedLevels)) {
            return $this->denyAccess('Users with your level are not allowed to unmark answers as correct.');
        }
        ...
        if ($userLevel >= $answerUserLevel) {
            return $this->denyAccess('You can only unmark answers of users with a lower level as correct.');
        }
